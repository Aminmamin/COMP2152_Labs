...
def log_to_csv(filename, command, target, result, status):
    ...
    with open(filename, "a", newline="") as file:
        ...
        writer.writerow([timestamp, command, target, str(result), status])
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            data = parse_nslookup(output)
            log_to_csv(LOG_FILE, "nslookup", domain, data["ip"], data["status"])

        elif choice == "3":
            info = parse_mac_address(get_network_info())
            log_to_csv(LOG_FILE, "ipconfig", "all", info["mac"] + " / " + info["ip"], "Captured")

        elif choice == "4":
            devices = parse_arp_table(get_arp_table())
            log_to_csv(LOG_FILE, "arp", "local", f"{len(devices)} devices", "Captured")
